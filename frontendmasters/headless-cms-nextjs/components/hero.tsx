...
export default async function Hero() {
  ...
  const data = await getHero(isDraft)
  const content = data.heroCollection.items[0]
  const ctas = {
    primary: content.callToActtionsCollection.items[0],
    secondary: content.callToActtionsCollection.items[1],
  }
        ...
        <div className="pt-32 pb-16 md:pt-52 md:pb-32">
          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="max-w-xs mx-auto space-y-4 sm:max-w-none sm:inline-flex sm:justify-center sm:space-y-0 sm:space-x-4" data-aos="fade-down" data-aos-delay="400">
              <div>
                <a className="w-full transition duration-150 ease-in-out btn text-slate-900 bg-gradient-to-r from-white/80 via-white to-white/80 hover:bg-white group" href={ctas.primary.url}>
                  {ctas.primary.label} <span className="tracking-normal text-purple-500 group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">-&gt;</span>
                </a>
              </div>
              <div>
                <a className="w-full transition duration-150 ease-in-out bg-opacity-25 btn text-slate-200 hover:text-white bg-slate-900 hover:bg-opacity-30" href={ctas.secondary.url}>
